...
params.ampThreshold = 0.3;    % fragment cut-off as a fraction of max(sumUnn)
...
for i_nImp = params.impulses
    ...
    mask = sumUnn >= max(sumUnn) * params.ampThreshold;
...
if params.runResonanceSweep
    ...
    maskSR   = sumUnnSR >= max(sumUnnSR) * params.ampThreshold;
...
function s = analyzeSignal(x, sig, mask, polyDeg, opts)
    ...
    pol = polyval(s.p_coef, s.x_frag, [], s.mu);
    if opts.Normalize
        pol = pol ./ max(pol);
    end
    s.pol = pol;
    ...
    % extrema of the polynomial
    minTF = islocalmin(s.pol, ext{:});
    maxTF = islocalmax(s.pol, ext{:});
    s.x_min = s.x_frag(minTF);  s.y_min = s.pol(minTF);
    s.x_max = s.x_frag(maxTF);  s.y_max = s.pol(maxTF);

    % exact zero of the derivative (= polynomial minimum). Normalisation is a
    % constant scale, so it does not move the zero; we locate it with the raw
    % derivative coefficients and read y off the normalised polynomial.
    d1 = @(xi) polyval(s.p_coef_d1, (xi - s.mu(1)) ./ s.mu(2));
    if ~isempty(s.x_min)
        s.fzero_x = fzero(d1, s.x_min);
        s.fzero_y = interp1(s.x_frag, s.pol, s.fzero_x, 'makima');
    else
        s.fzero_x = NaN;
        s.fzero_y = NaN;
    end

...
if params.runResonanceSweep
    ...
    for ip = 1:numel(sr.pdfs)
        [mn, iStar] = min(sr.rmse(ip, :));
        fprintf(' %-5s: empirical σ* = %.6f (%.3f·Δ), min RMSE = %.6f;  canonical σ = %.3f·Δ\n', ...
                upper(sr.pdfs{ip}), sr.sigma(iStar), sr.sigma(iStar)/sr.Delta, ...
                mn, sr.sigmaCanon(ip)/sr.Delta);
    end

    % dither budget: TPDF canonical point, after subtracting measured noise
    sigma_d = ditherForResonance(sr.sigmaCanon(2), params.srNoiseSigma);
...
function sr = sweepResonance(x, sig, mask, params)
    ...
    rmse   = nan(numel(pdfs), params.srNumSigma);
    bias   = nan(numel(pdfs), params.srNumSigma);
    sd     = nan(numel(pdfs), params.srNumSigma);
    validN = zeros(numel(pdfs), params.srNumSigma);
    ...
    for ip = 1:numel(pdfs)
        for is = 1:params.srNumSigma
            ...
            for m = 1:M
                ...
                rec = mean(quantize(sig + dN, Delta), 1);     % average quantized copies
                s   = analyzeSignal(x, rec, mask, params.polyDeg);
                xhat(m) = s.fzero_x;
            end
            good = xhat(isfinite(xhat));
            validN(ip, is) = numel(good);
            if ~isempty(good)
                err          = good - x_true;
                rmse(ip, is) = sqrt(mean(err.^2));
                bias(ip, is) = mean(err);
                sd(ip, is)   = std(good);
            end
        end
    ...
    sr.sigmaCanon = Delta * [1/sqrt(12), 1/sqrt(6), 1/2];
end
...
% Plot the linearisation curves (RMSE vs σ/Δ) for the three dither PDFs,
% marking each PDF's canonical operating point on its own curve.
function plotResonance(sr, iSR)
    figure('Name', sprintf('Dither PDF comparison — impulse %d', iSR));
    cols = {'#d62728', '#431e61', '#5abbe8'};       % rpdf, tpdf, gauss
    hold on; grid on;
    for ip = 1:numel(sr.pdfs)
        plot(sr.sigma / sr.Delta, sr.rmse(ip, :), '-o', 'Color', cols{ip}, ...
             'LineWidth', 1.5, 'MarkerFaceColor', cols{ip}, ...
             'DisplayName', upper(sr.pdfs{ip}));
        % canonical operating point of this PDF, interpolated onto its curve
        sc = sr.sigmaCanon(ip);
        yc = interp1(sr.sigma, sr.rmse(ip, :), sc, 'linear');
        plot(sc / sr.Delta, yc, 'p', 'MarkerSize', 14, 'Color', cols{ip}, ...
             'MarkerFaceColor', cols{ip}, ...
             'DisplayName', sprintf('%s canonical \\sigma', upper(sr.pdfs{ip})));
    end
    xlabel('dither RMS  \sigma / \Delta');
    ylabel('position RMSE');
    title(sprintf('Non-subtractive dither: PDF comparison (impulse %d)', iSR));
